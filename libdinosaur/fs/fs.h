...
class FileEventInterface : public std::enable_shared_from_this<FileEventInterface>
{
public:
	typedef std::weak_ptr<FileEventInterface> ptr;
							FileEventInterface(){}
	virtual 				~FileEventInterface(){}
...
private:
	...
	std::string				m_fn;
	int 					m_fd;
	FileEventInterface::ptr 	m_assoc;
	bool 					m_instance2delete;
	void 					event_file_write(const write_event & eo)
	{
		if (!m_assoc.expired())
			m_assoc.lock()->event_file_write(eo);
	}
public:
							file();
							file(const char * fn, uint64_t length, bool should_exists, const FileEventInterface::ptr & assoc) throw(Exception);
	virtual					~file();
...
public:
	...
	int 					File_add(const char * fn, uint64_t length, bool should_exists, const FileEventInterface::ptr & assoc, File & file) throw (Exception);
	int 					File_add(const std::string & fn, uint64_t length, bool should_exists, const FileEventInterface::ptr & assoc, File & file) throw (Exception);
