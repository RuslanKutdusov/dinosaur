/*
 * consts.h
 *
 *  Created on: 17.02.2012
 *      Author: ruslan
 */

#ifndef CONSTS_H_
#define CONSTS_H_

#define MAX_FILENAME_LENGTH 2048
#define STATE_FILE_VERSION 1
#define CONFIG_FILE_VERSION 3
#define CLIENT_ID "-DI-0001-35345343441"
#define BLOCK_LENGTH 16384
#define HANDSHAKE_LENGHT 68
#define SHA1_LENGTH 20
//#define SHA1_HEX_LENGTH 40
#define MAX_TORRENT_FILE_LENGTH 10485760
#define MAX_RELEASE_TIME 60
#define NODE_ID_LENGTH 20

#define PEER_ID_LENGTH 20
#define PEER_SLEEP_TIME 60
#define PEER_MAX_REQUEST_NUMBER 64
#define IP_CHAR_LENGHT 16 //xxx.xxx.xxx.xxx\0
#define EXCEPTIONS_DESCRIPTIONS_FILE "exceptions.xml"
#define ERASABLE_LOOP(CONTAINER, ITER_TYPE, ERASABLE_ITER, INC_ITER) for(ITER_TYPE ERASABLE_ITER = CONTAINER.begin(), INC_ITER = ERASABLE_ITER; ERASABLE_ITER != CONTAINER.end(); ERASABLE_ITER = INC_ITER)


#endif /* CONSTS_H_ */
